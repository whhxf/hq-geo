...
2. `data/keywords.csv` 中有 `status = "content_created"` 或 `"published"` 的关键词（已有内容才值得监控）
3. `data/competitors.csv` 已有竞争分析数据（用于对比趋势）

若条件不满足，监控仍可运行但结果无意义（brand_mentioned 全部为 false）。应先引导用户完成前置步骤。

## 启动时机

监控是**用户触发型**操作，以下用户表达时启动：
- "运行监控"、"监控一下"、"今天被引用了吗"、"看看我们的表现"、"跑一次监控"

也可在**内容发布后**主动建议用户运行一次监控，验证内容效果。
