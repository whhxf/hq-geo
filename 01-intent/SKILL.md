...
如果 `data/brand.csv` 仍为占位符（包含 `[` 和 `]`），引导用户完成 3 步初始化：

**第 1 步：选择目标市场**
```
请选择目标市场：
  [1] 国内 GEO（中文，面向中国大陆市场）→ market=domestic
  [2] 国外 GEO（英文/多语言，面向海外市场）→ market=foreign
  [3] 双市场（同时覆盖国内和国外）→ market=both
```
...
- 品牌/公司名称 → brand_name
- 行业/品类 → industry
- 核心产品/服务 → product_description
- 目标客户(ICP) → target_customer
- 独特优势/价值主张 → core_value_prop
- 官网地址 → website_url
- 竞品列表（分号分隔） → competitors
- 目标 AI 平台（逗号分隔） → geo_target_platforms
- 内容语言（zh/en） → language

**第 3 步：确认并保存**
将上述信息写入 `data/brand.csv`（键值对格式：field,value），确认后进入 Step 1。

### Step 1：读取品牌档案
读取 `data/brand.csv`，提取以下字段：
- brand_name、industry、product_description、target_customer、core_value_prop、competitors、language、market

如果关键字段为空，先暂停并询问用户补充，或执行 Step 0 引导用户填写 brand.csv。

读取 `market` 字段决定关键词生成策略：
- `domestic` → 中文关键词，面向豆包/DeepSeek/千问等平台
- `foreign` → 英文关键词，面向 ChatGPT/Perplexity/Gemini 等平台
- `both` → 生成中英文两套关键词，分别标记 market 字段

### Step 2：生成四类关键词

根据 brand.csv 中的 `market` 字段决定生成策略：
- `domestic` → 中文关键词，口语化，符合豆包/DeepSeek/千问用户提问习惯
- `foreign` → 英文关键词，符合 ChatGPT/Perplexity/Gemini 用户提问习惯
- `both` → 生成中英文两套关键词，中文标记 market=domestic，英文标记 market=foreign
...
- 不要生成英文关键词（除非 brand.csv 中 market=foreign 或 language=en）
